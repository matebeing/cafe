    ...
    cafeSocket.on('connection', function(socket){
        ...
        socket.on("createTopic", async (data) => {
            ...
            if(isValid) {
                ...
                discussions.push({id: topicId, title: data.topic.title, owner: user.username, avatar: user.avatar, content: [{topicID: topicId, id: 0, content: data.topic.content[0].content, owner: user.username, avatar: user.avatar, colorName: user.colorName}], lastComment: data.topic.lastComment, date: data.topic.date})
        ...
        socket.on("newComment", async (data) => {
            ...
            discussions[data.topicId].content.push({topicID: data.topicId, id: discussions[data.topicId].content[discussions[data.topicId].content.length - 1].id + 1, content: data.content, owner: user.username, avatar: user.avatar, colorName: user.colorName})
